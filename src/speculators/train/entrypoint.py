...
class _TrainingSession:
    """The initialized trainer and model that together own a training run."""
...
def _build_training_session(cfg: TrainConfig) -> _TrainingSession:  # noqa: C901
    ...
    if args.dry_run:
        ...
        maybe_destroy_distributed()
        return _TrainingSession(trainer=None, draft_model=None)
...
def main(argv: Sequence[str] | None = None) -> int:
    run_training(TrainConfig.resolve(list(argv) if argv is not None else None))
    return 0
...
# RUN WITH:
# torchrun --standalone --nproc_per_node=<num_gpus> -m speculators.train
# for multi-GPU training (DDP by default)
# OR
# torchrun --standalone --nproc_per_node=<num_gpus> -m speculators.train --fsdp-shard
